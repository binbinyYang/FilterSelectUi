...
import com.yangbin.listener.OnPopupDismissListener;
import com.yangbin.listener.OnSelectFilterNameListener;
import com.yangbin.listener.OnSelectResultListener;
import com.yangbin.listener.PopupEntityLoaderImp;
...
public class FilterTabView extends LinearLayout implements OnFilterToViewListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            btnTextSelect = a.getColor(R.styleable.FilterTabView_btn_text_select_color, mContext.getResources().getColor(R.color.color_ff8800));
            btnTextUnSelect = a.getColor(R.styleable.FilterTabView_btn_text_unselect_color, mContext.getResources().getColor(R.color.color_ff8800));
...
//           SpUtils.getInstance(context).putCornerRadius(btnCornerRadius);
            //如果是矩形
            SpUtils.getInstance(context).putCornerRadius(0);
            //如果是四个角有弧度的
//            SpUtils.getInstance(context).putCornerRadius(8);
            // button 字体颜色
...
//            tv_tab.setTextColor(btnTextUnSelect);
    ...
    public void setOnAdapterRefreshListener(AreaSelectPopupWindow onAdapterRefreshListener) {
        this.onAdapterRefreshListener = onAdapterRefreshListener;
    }

    @Override
    public void onFilterToView(FilterResultBean resultBean, int postion) {

        int popupIndex = resultBean.getPopupIndex();

        if (resultBean.getPopupType() == FilterTabConfig.FILTER_TYPE_SINGLE_SELECT_HAVA_PIC) {
...
//                mTextViewLists.get(popupIndex).setTextColor(getResources().getColor(R.color.black));
                mTextViewLists.get(popupIndex).setBackgroundDrawable(getResources().getDrawable(R.drawable.icon_commercial_real_sort_normal));
